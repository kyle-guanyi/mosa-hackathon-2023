...
const MultiSelectDropdown: React.FC<MultiSelectDropdownProps> = ({ options, selected = [], onSelectedChange }) => {
            ...
            {isOpen && (
                <ul className="absolute left-0 mt-2 w-48 bg-white rounded shadow-md overflow-y-auto max-h-48 z-50">
                    {options.map((option, idx) => (
                        <li key={idx} className="flex items-center px-4 py-2 hover:bg-blue-500 hover:text-white">
                            <input
                                type="checkbox"
